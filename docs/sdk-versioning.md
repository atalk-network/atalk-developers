...
- npm packages: `0.1.0-alpha.3`, distributed under the `next` tag;
- Python package: `0.1.0a3`, installed with `pip --pre`;
- Git release tags: `node-v0.1.0-alpha.3` and `python-v0.1.0a3`.
...
`@atalk/sdk`, `@atalk/protocol`, `@atalk/core-native` and every native platform package share one exact npm version. The Node release workflow publishes them as one release set and verifies the native artifact matrix before publication.
